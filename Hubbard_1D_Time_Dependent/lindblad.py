...
import sys
class Lindblad:
    def __init__(self, H_op, H_op_2, A_op, A_op_2):
        self.H_op = H_op
        self.H_op_2 = H_op_2
        self.A_kraus_full = A_op
        self.A_kraus_full_2 = A_op_2
    ...
    def Lindblad_exact(self,T,num_t,psi0):
        ...
        V_full_2=np.zeros((Ns,Ns,K),dtype=complex)
        for i in range(K):
            V_full[:,:,i]=self.A_kraus_full[:,:,i]
            V_full_2[:,:,i]=self.A_kraus_full_2[:,:,i]
        tau = T / num_t
        ...
        for it in range(num_t):
            ...
            rho_K[:,:,0]=-1j*((H+tau*it*H_2)@rho_mid-rho_mid@(H+tau*it*H_2))
            for i in range(K):
               rho_K[:,:,0]+=(V_full[:,:,i]+tau*it*V_full_2[:,:,i])@rho_mid@(V_full[:,:,i]+tau*it*V_full_2[:,:,i]).conj().T\
                -1/2*((V_full[:,:,i]+tau*it*V_full_2[:,:,i]).conj().T\
                      @ (V_full[:,:,i]+tau*it*V_full_2[:,:,i])@rho_mid+rho_mid@(V_full[:,:,i]+tau*it*V_full_2[:,:,i]).conj().T\
                      @ (V_full[:,:,i]+tau*it*V_full_2[:,:,i]))
            rho_mid=rho[:,:,it]+tau/2*rho_K[:,:,0]
            rho_K[:,:,1]=-1j*((H+tau*(it+1/2)*H_2)@rho_mid-rho_mid@(H+tau*(it+1/2)*H_2))
            for i in range(K):
               rho_K[:,:,1]+=(V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i])@rho_mid@(V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i]).conj().T\
                -1/2*((V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i]).conj().T\
                      @ (V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i])@rho_mid\
                      +rho_mid@(V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i]).conj().T\
                      @ (V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i]))
            rho_mid=rho[:,:,it]+tau/2*rho_K[:,:,1]
            rho_K[:,:,2]=-1j*((H+tau*(it+1/2)*H_2)@rho_mid-rho_mid@(H+tau*(it+1/2)*H_2))
            for i in range(K):
               rho_K[:,:,2]+=(V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i])@rho_mid@(V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i]).conj().T\
                -1/2*((V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i]).conj().T\
                      @ (V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i])@rho_mid\
                      +rho_mid@(V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i]).conj().T\
                      @ (V_full[:,:,i]+tau*(it+1/2)*V_full_2[:,:,i]))
            rho_mid=rho[:,:,it]+tau*rho_K[:,:,2]
            rho_K[:,:,3]=-1j*((H+tau*(it+1)*H_2)@rho_mid-rho_mid@(H+tau*(it+1)*H_2))
            for i in range(K):
               rho_K[:,:,3]+=(V_full[:,:,i]+tau*(it+1)*V_full_2[:,:,i])@rho_mid@(V_full[:,:,i]+tau*(it+1)*V_full_2[:,:,i]).conj().T\
                -1/2*((V_full[:,:,i]+tau*(it+1)*V_full_2[:,:,i]).conj().T\
                      @ (V_full[:,:,i]+tau*(it+1)*V_full_2[:,:,i])@rho_mid\
                      +rho_mid@(V_full[:,:,i]+tau*(it+1)*V_full_2[:,:,i]).conj().T\
                      @ (V_full[:,:,i]+tau*(it+1)*V_full_2[:,:,i]))
    ...
    def Lindblad_H(self, T, num_t, psi0,order): 
        ...
        for it in range(num_t):
            ...
            H_2_star=np.zeros((Ns,Ns,K),dtype=complex)
            ...
            G_de=np.zeros((Ns,Ns),dtype=complex)
            G_sd=np.zeros((Ns,Ns),dtype=complex)
            for i in range(K):
                G0+=-1/2*(V_full[:,:,i]+tau*it*V_full_2[:,:,i]).conj().T@(V_full[:,:,i]+tau*it*V_full_2[:,:,i]) #Jump operator part of V0；
                G_de+=-1/2*V_full_2[:,:,i].conj().T@(V_full[:,:,i]+tau*it*V_full_2[:,:,i])\
                -1/2*(V_full[:,:,i]+tau*it*V_full_2[:,:,i]).conj().T@V_full_2[:,:,i]           #deraivative of G0；
                G_sd+=-V_full_2[:,:,i].conj().T@V_full_2[:,:,i]
            G=-1j*(H+tau*it*H_2)+G0   #V0；
            G_de=-1j*H_2+G_de   #Deraivative of V0；
            #F0= eye(size(G)) + G*dt + 1/2*G^2*dt^2+1/6*G^3*dt^3;
            Y_0[:,:,0,0]= G; Y_0[:,:,0,1]=(G@G+G_de)/2; Y_0[:,:,0,2]= (G@G@G+2*G_de@G+G@G_de+G_sd)/6
            for i in range(K):
                V=V_full[:,:,i]+tau*it*V_full_2[:,:,i]
                V_de=V_full_2[:,:,i]
                #F1= -1i*( V + 1/2*V*G*dt+1/2*G*V*dt + (G^2*V+V*G^2+G*V*G)*dt^2/6 )*sqrt(dt);
                Y_1[:,:,i,0]=V; Y_1[:,:,i,1]= 1/2*(V_de+V@G+G@V); Y_1[:,:,i,2]= (G@G@V+2*G_de@V+G@V@G+G@V_de+V@G@G+2*V_de@G+V@G_de)/6
                #F2= -1i*(G*V - V*G)*dt*sqrt(dt)/sqrt(12);
                Y_2[:,:,i,1]= (G@V-V@G-V_de)/np.sqrt(12)
            index=list(product(range(K),repeat=3))
            for i in range(len(index)):
                V_1=V_full[:,:,index[i][0]]+tau*it*V_full_2[:,:,index[i][0]]
                V_2=V_full[:,:,index[i][1]]+tau*it*V_full_2[:,:,index[i][1]]
                V_3=V_full[:,:,index[i][2]]+tau*it*V_full_2[:,:,index[i][2]]
            ...
            for i in range(len(index)):
                ...
                V_1=V_full[:,:,index[i][0]]+tau*it*V_full_2[:,:,index[i][0]]
                V_de_1=V_full_2[:,:,index[i][0]]
                V_2=V_full[:,:,index[i][1]]+tau*it*V_full_2[:,:,index[i][1]]
                V_de_2=V_full_2[:,:,index[i][1]]
                Y_3[:,:,index[i][0],index[i][1],0]= V_1@V_2/np.sqrt(2)
                Y_3[:,:,index[i][0],index[i][1],1]= np.sqrt(2)/6*(G@V_1@V_2+V_1@G@V_2+V_1@V_2@G+2*V_de_1@V_2+V_1@V_de_2)
    ...
    #         print(H_0[:,:,0])
            ...
            for i in range(K):
                ...
                H_2_star[:,:,i] = X_2[:,:,i,0] + tau*X_2[:,:,i,1]*int(order>=2)+ tau*tau*X_2[:,:,i,2]*int(order>=3)
    #         print(H_1[:,:,0])
    #         print(H_2[:,:,0])
            index=list(product(range(K),repeat=3))
            for i in range(len(index)):
    ...
    #         print(H_3[:,:,0,0])    
            ...
            for i in range(K):
                ...
                H_large[((i+K+1)*Ns):((i+K+2)*Ns),:Ns]=H_2_star[:,:,i]
            index=list(product(range(K),repeat=3))
            for i in range(len(index)):
    ...
    #         exp_V=la.expm(-1j*np.sqrt(tau)*V_full)
...
#             G_sd=np.zeros((Ns,Ns),dtype=complex)
#             for i in range(K):
#                 G0+=-1/2*(V_full[:,:,i]+tau*it*V_full_2[:,:,i]).conj().T@(V_full[:,:,i]+tau*it*V_full_2[:,:,i])
#                 G_de+=-1/2*V_full_2[:,:,i].conj().T@(V_full[:,:,i]+tau*it*V_full_2[:,:,i])\
#                 -1/2*(V_full[:,:,i]+tau*it*V_full_2[:,:,i]).conj().T@V_full_2[:,:,i]
#                 G_sd+=-V_full_2[:,:,i].conj().T@V_full_2[:,:,i]
#             G=-1j*(H+tau*it*H_2)+G0
#             G_de=-1j*H_2+G_de
#             F_0[:,:,0]=np.eye(Ns,dtype=complex)+G*tau+1/2*(G@G+G_de)*tau**2+1/6*(G@G@G+2*G_de@G+G@G_de+G_sd)*tau**3
#             for i in range(K):
#                 V=V_full[:,:,i]+tau*it*V_full_2[:,:,i]
#                 V_de=V_full_2[:,:,i]
#                 F_1[:,:,i]=np.sqrt(tau)*(V+tau/2*(V_de+V@G+G@V)+tau**2/6*(G@G@V+2*G_de@V+G@V@G+G@V_de+V@G@G+2*V_de@G+V@G_de))
#                 F_2[:,:,i]=np.sqrt(tau)*tau/np.sqrt(12)*(G@V-V@G-V_de)
#             index=list(product(range(K),repeat=2))
#             for i in range(len(index)):
#                 V_1=V_full[:,:,index[i][0]]+tau*it*V_full_2[:,:,index[i][0]]
#                 V_de_1=V_full_2[:,:,index[i][0]]
#                 V_2=V_full[:,:,index[i][1]]+tau*it*V_full_2[:,:,index[i][1]]
#                 V_de_2=V_full_2[:,:,index[i][1]]
#                 F_3[:,:,index[i][0],index[i][1]]=np.sqrt(2)*tau*(1/2*V_1@V_2+tau/6*(G@V_1@V_2+V_1@G@V_2+V_1@V_2@G+2*V_de_1@V_2+V_1@V_de_2))
#             index=list(product(range(K),repeat=3))
#             for i in range(len(index)):
#                 V_1=V_full[:,:,index[i][0]]+tau*it*V_full_2[:,:,index[i][0]]
#                 V_2=V_full[:,:,index[i][1]]+tau*it*V_full_2[:,:,index[i][1]]
